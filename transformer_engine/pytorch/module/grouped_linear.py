        ...
        with torch.cuda.nvtx.range("_GroupedLinear_backward"):
            ...
            if ctx.weights_requires_grad:
                ...
                if ctx.save_original_input:
                    with_multi_quantize_split = False
                    if ctx.input_quantizers[0] is not None:
                        ...
                        with_multi_quantize_split = (
                            ctx.fp8 and isinstance(ctx.input_quantizers[0], Float8BlockQuantizer)
                        )
                    inp = inputmats[0]
                    in_features = inp.shape[-1]
                    inp_view = inp.view(-1, in_features)
                    if with_multi_quantize_split:
                        inp_view = cast_if_needed(inp_view, ctx.activation_dtype)
                        inputmats_no_fp8 = []
                    else:
